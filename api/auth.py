...
SECRET_KEY = "8bb102f260c30525c5f35c559518d2c7ceb488f0a37fae2eda5bd74c24e0cb36"
...
def create_user_account(username, password, days_valid=36500, created_by="admin"):
...
def authenticate_user(username, password):
    ...
    for user in users_data['users']:
        if user['username'] == username:
            # Accounts are always active - manual management only
...
def verify_user_token(token):
    ...
    for session in sessions_data['sessions']:
        if session['token'] == token:
            ...
            if not last_activity or (current_time - datetime.fromisoformat(last_activity).replace(tzinfo=timezone.utc)).seconds > 300:  # Update every 5 minutes
