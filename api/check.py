    ...
    def do_POST(self):
        try:
            ...
            if action == 'start':
                ...
                all_results = checker_state['results']
                valid_cookies = [r for r in all_results if r['status'] == 'valid']

                export_data = "# ROBLOX COOKIES CHECK RESULTS\n"
                export_data += f"# Generated: {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')}\n"
                export_data += f"# Total Checked: {len(all_results)}\n"
                export_data += f"# Total Valid: {len(valid_cookies)}\n"
                export_data += f"# Total Invalid: {len(all_results) - len(valid_cookies)}\n"
                ...
                # Export all results, marking valid ones
                for i, result in enumerate(all_results):
                    status_label = "VALID" if result['status'] == 'valid' else "INVALID"
                    export_data += f"=== {status_label} ACCOUNT {i+1} ===\n"
                    export_data += f"Status: {result['status']}\n"
                    export_data += f"Username: {result['username']}\n"
                    export_data += f"Display Name: {result['display_name']}\n"
                    export_data += f"User ID: {result['user_id']}\n"
                    if result['status'] == 'valid':
                        export_data += f"Robux: {result.get('robux', 0)}\n"
                        export_data += f"Premium: {'Yes' if result.get('premium', False) else 'No'}\n"
                        export_data += f"Friends: {result.get('friends_count', 0)}\n"
                        export_data += f"Created: {result.get('created_date', 'Unknown')}\n"
                    if result.get('error'):
                        export_data += f"Error: {result['error']}\n"
                    export_data += f"Checked: {result['timestamp']}\n"
                    export_data += "---------------------------------------\n"
                    export_data += f"{result.get('cookie', 'N/A')}\n\n"

                response = {
                    'success': True,
                    'export_data': export_data,
                    'filename': f'cookie_check_results_{datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")}.txt'
                }
